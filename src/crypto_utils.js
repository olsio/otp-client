...
const prepareCounter = counter => {
  ...
  return new Buffer(paddedCounter, "hex")
}

const toBase32 = value => {
  return base32.encode(value).toString().replace(/=/g, "")
}
...
const padSecret = (secret, algorithm) => {
  const encoded = new Buffer(secret, "hex")
...
const padBuffer = (secretBuffer, size) => {
  ...
  return new Buffer(secretBuffer, "hex").slice(0, size)
}
